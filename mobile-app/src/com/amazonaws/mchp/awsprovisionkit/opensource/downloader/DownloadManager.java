...
import com.amazonaws.mchp.awsprovisionkit.utils.LogUtils;
            ...
            public void run() {
                ...
                switch (message) {
                    ...
                    case STOP:
                        ...
                        break;
                    case ERROR:
                        listener.onError(task,new DownloadException(code));
                        break;
                }
            }
        });
...
//                Log.i("The Task is already stored in the sqlite.");
    ...
    public void delete(DownloadTask task, DownloadListener listener) {
        ...
        switch (task.getStatus()) {
            case DownloadStatus.STATUS_DELETED:
                ...
                LogUtils.i("The Task is already Deleted.");
                break;
            default:
                task.delete();
                OnResult(POST_MESSAGE.DELETE, task, listener, -1);
     ...
     * @return
     */
    public void deleteforever(DownloadTask task, DownloadListener listener) throws SQLException {
    	LogUtils.i("Delete Task forever");
...
//            iSql.deleteDownloadTask(task);
     ...
     */
    @SuppressWarnings("rawtypes")
    public boolean start(DownloadTask task, DownloadListener listener) throws SQLException {
    	LogUtils.i("Start Task");
...
//        ISql iSql = new ISqlImpl(context);
		...
		switch (task.getStatus()) {
		    case DownloadStatus.STATUS_RUNNING:
		        ...
		        LogUtils.i("The Task is already Running.");
		        break;
		    default:
		        if (listener != null) {
		            task.start(context, listener, false);
     ...
     * @return
     */
    public void stop(DownloadTask task, DownloadListener listener) throws SQLException {
    	LogUtils.i("Stop Task");
        ...
        switch (task.getStatus()) {
            case DownloadStatus.STATUS_STOPPED:
                ...
                LogUtils.i("The Task is already Stopped.");
                break;
            case DownloadStatus.STATUS_RUNNING:
